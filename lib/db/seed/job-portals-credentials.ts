...
/**
 * Seed job portals, their credentials, and sample evaluations
 */
export async function seedJobPortalsAndCredentials() {
  ...
  try {
    ...
    if (existingCategory) {
      ...
      results.push('✓ Job Portals category already exists')
    } else {
      ...
      results.push('✓ Created Job Portals category')
    }
    ...
    for (const portal of jobPortalsData) {
      ...
      if (existingCred) {
        ...
        results.push(`✓ Credential "${portal.name}" already exists`)
      } else {
        ...
        if (credError) {
          results.push(`✗ Failed to create credential "${portal.name}": ${credError.message}`)
          continue
        }
        credentialId = credential.id
        results.push(`✓ Created credential "${portal.name}"`)
      }
      ...
      if (existingPortal) {
        ...
        results.push(`✓ Job portal "${portal.name}" already exists`)
      } else {
        ...
        if (portalError) {
          results.push(`✗ Failed to create job portal "${portal.name}": ${portalError.message}`)
          continue
        }
        portalId = jobPortal.id
        results.push(`✓ Created job portal "${portal.name}"`)
      }
      ...
      if (!existingLink) {
        ...
        if (linkError) {
          results.push(`✗ Failed to link credential to portal "${portal.name}": ${linkError.message}`)
        } else {
          results.push(`✓ Linked credential to job portal "${portal.name}"`)
        }
      } else {
        results.push(`✓ Credential already linked to "${portal.name}"`)
      }
    }

    return {
      success: true,
      message: 'Job portals and credentials seeded successfully',
      details: results,
    }
  } catch (error) {
    return {
...
/**
 * Seed sample evaluations for existing interviews
 */
export async function seedEvaluations() {
  ...
  try {
    ...
    if (!interviews || interviews.length === 0) {
      results.push('No completed interviews found to evaluate')
      return { success: true, message: 'No interviews to evaluate', details: results }
    }
    ...
    for (const interview of interviews) {
      ...
      if (evalError) {
        results.push(`✗ Failed to create evaluation for ${candidateName}: ${evalError.message}`)
      } else {
        results.push(`✓ Created evaluation for ${candidateName}`)
        created++
      }

      if (created >= 10) break // Limit to 10 evaluations
    }

    if (created === 0) {
      results.push('All completed interviews already have evaluations')
    }

    return {
      success: true,
      message: `Created ${created} evaluations`,
      details: results,
    }
  } catch (error) {
    return {
...
export async function seedAll() {
  const portalsResult = await seedJobPortalsAndCredentials()
  const evaluationsResult = await seedEvaluations()

  return {
    jobPortals: portalsResult,
