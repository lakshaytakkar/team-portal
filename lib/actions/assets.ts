    ...
    if (employeeData?.profile_id && assetData) {
      try {
        await createNotification({
          ...
          message: `You have been assigned ${assetData.name}${assetData.asset_type ? ` (${assetData.asset_type.name})` : ''}`,
          data: {
            asset_id: assetId,
            asset_name: assetData.name,
            asset_type: assetData.asset_type?.name,
            employee_id: employeeId,
            assigned_date: assignedDate,
          },
        })
    ...
    if (employeeData?.profile_id && assetData) {
      try {
        await createNotification({
          ...
          message: `Asset ${assetData.name}${assetData.asset_type ? ` (${assetData.asset_type.name})` : ''} has been returned`,
          data: {
