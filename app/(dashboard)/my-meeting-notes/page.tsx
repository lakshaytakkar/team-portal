...
import { FileText, Plus, Search, Calendar, Users, Tag, Edit, Trash2 } from "lucide-react"
...
export default function MyMeetingNotesPage() {
  ...
  const { data: notes, isLoading, error, refetch } = useQuery({
    queryKey: ["my-meeting-notes", filters, user?.id],
    queryFn: async () => {
      if (!user?.id) return []
      return await getMeetingNotes(filters, user.id)
    },
  ...
  if (error) {
    return (
      <ErrorState
        ...
        message="We couldn't load your meeting notes. Please check your connection and try again."
        onRetry={() => refetch()}
      />
    )
  }
          ...
          {notes && notes.length > 0 ? (
            notes.map((note) => (
              <div
                ...
                onClick={() => router.push(`/my-meeting-notes/${note.id}`)}
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div onClick={(e) => e.stopPropagation()}>
                    <RowActionsMenu
                      ...
                      detailUrl={`/my-meeting-notes/${note.id}`}
                      canEdit={true}
                      canDelete={true}
              ...
              <EmptyState
                ...
                description={
                  searchQuery
                    ...
                    : "You haven't created any meeting notes yet."
                }
              />
            </div>
          )}
