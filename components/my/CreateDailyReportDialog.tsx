...
export function CreateDailyReportDialog({ open, onOpenChange }: CreateDailyReportDialogProps) {
  const [formData, setFormData] = useState({
    date: "", tasksCompleted: "", hoursWorked: "",
    notes: "", blockers: "", nextDayPlan: "", projectsWorkedOn: "",
  })

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      console.log("Create daily report:", formData)
      toast.success("Daily report submitted successfully", { description: "Your daily report has been submitted", duration: 3000 })
      onOpenChange(false)
      setFormData({ date: "", tasksCompleted: "", hoursWorked: "", notes: "", blockers: "", nextDayPlan: "", projectsWorkedOn: "" })
    } catch (error) {
      toast.error("Failed to submit report", { description: error instanceof Error ? error.message : "An error occurred", duration: 5000 })
    }
  }
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] flex flex-col p-0">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto px-6 space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium text-[#666d80] leading-[1.5] tracking-[0.28px]">
                Tasks Completed <span className="text-[#df1c41]">*</span>
              </Label>
              <Textarea
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium text-[#666d80] leading-[1.5] tracking-[0.28px]">
                Hours Worked <span className="text-[#df1c41]">*</span>
              </Label>
              <Input
                type="number"
            ...
            <Accordion type="single" collapsible className="w-full">
              <AccordionItem value="additional">
                ...
                <AccordionContent className="space-y-4 pt-4">
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-[#666d80] leading-[1.5] tracking-[0.28px]">Blockers</Label>
                    <Textarea
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-[#666d80] leading-[1.5] tracking-[0.28px]">Next Day Plan</Label>
                    <Textarea
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-[#666d80] leading-[1.5] tracking-[0.28px]">Projects Worked On</Label>
                    <Input
                      value={formData.projectsWorkedOn}
                      onChange={(e) => setFormData({ ...formData, projectsWorkedOn: e.target.value })}
